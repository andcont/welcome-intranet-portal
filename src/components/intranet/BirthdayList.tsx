...
const BirthdayList = () => {
  ...
  const loadData = async () => {
    try {
      console.log('BirthdayList - Loading data...');
      
      const [profilesResult, departmentsResult] = await Promise.all([
        supabase
          .from('profiles')
          .select('id, name, email, role, department_id, profile_image, birthday')
          .order('name'),
      ...
      if (profilesResult.error) {
        ...
        console.log('BirthdayList - Profiles loaded:', profilesResult.data);
        console.log('BirthdayList - Profiles with images:', profilesResult.data?.filter(p => p.profile_image));
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {todaysBirthdays.map((member) => {
                console.log('BirthdayList - Today birthday member:', member.name, 'Image:', member.profile_image);
                return (
          ...
          <CardContent>
            <div className="space-y-3">
              {upcomingBirthdays.map((member) => {
                ...
                console.log('BirthdayList - Upcoming birthday member:', member.name, 'Image:', member.profile_image);
                return (
          ...
          {monthlyBirthdays.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {monthlyBirthdays.map((member) => {
                console.log('BirthdayList - Monthly birthday member:', member.name, 'Image:', member.profile_image);
                return (
